...
var skip = ref(0);
const products = ref([]);
var product_limit = ref(4);
var show_load_more = ref(true);
var loading_products = ref(false);
...
const getAllProducts = () => {
  ...
  setTimeout(() => {
    axios.get(`https://dummyjson.com/products?limit=${product_limit.value}&skip=${skip.value}`).then((response) => {
      skip.value = response.data.skip + response.data.limit;
      products.value = products.value.concat(response.data.products);
      loading_products.value = false;
      show_load_more.value = response.data.skip < response.data.total;
    });
  }, 2000)
}
</script>
  ...
  <section class="bg-gray-50 py-8 antialiased dark:bg-gray-900 md:py-12">
    <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
      ...
      <div class="mb-4 grid gap-4 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 xl:grid-cols-4">
          ...
          <div class="pt-6">
            ...
            <ul class="mt-2 flex items-center gap-4">
              <li class="flex items-center gap-2">
                ...
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ product.category }}</p>
              </li>
              <li class="flex items-center gap-2">
                <svg class="h-4 w-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                     xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        >
          <div class="h-56 w-full bg-gray-300 dark:bg-gray-700"></div>
          <div class="pt-6 space-y-3">
            <div class="h-5 w-3/4 bg-gray-300 dark:bg-gray-700"></div>
            <div class="h-4 w-1/2 bg-gray-300 dark:bg-gray-700"></div>
            <div class="h-5 w-full bg-gray-300 dark:bg-gray-700"></div>
          </div>
        </div>
      </div>
      <div class="w-full text-center" v-show="show_load_more">
        <button @click="getAllProducts" type="button"
                class="rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
                v-html="loading_products ? 'Fetching...' : 'Show More'">
        </button>
      </div>
    </div>
  </section>
